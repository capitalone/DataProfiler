...
@mock.patch('data_profiler.labelers.data_processing.BaseDataProcessor')
...
class TestDataLabeler(unittest.TestCase):
    ...
    def _setup_mock_load_processor(mock_base_processor):
        def side_effect(arg):
            processor = {
                'CharPreprocessor': mock.Mock(
                    spec=data_processing.CharPreprocessor),
    ...
    def test_load_from_library(self, mock_open, mock_load_model,
                               mock_base_processor):
    ...
    def test_valid_fit_data_formats(self, *mocks):
        ...
        def _valid_check(data):
            try:
                print("\tChecking data format: {}".format(str(type(data))))
                data = UnstructuredDataLabeler.\
                    _check_and_return_valid_data_format(
    ...
    def test_set_model(self, mock_open, mock_load_model, mock_load_processor,
        ...
        with self.assertRaisesRegex(TypeError,
                                    'The specified model was not of the correct'
                                    ' type, `BaseModel`.'):
    ...
    def test_set_postprocessor(self, mock_open, mock_load_model,
                               mock_load_processor, *mocks):
    ...
    def test_check_pipeline(self, mock_open, mock_load_model,
        ...
        with self.assertWarnsRegex(RuntimeWarning,
                                   'Model and preprocessor value for `a` do '
                                   'not match. 1 != 2'):
            data_labeler.check_pipeline()
        ...
        with self.assertWarnsRegex(RuntimeWarning,
                                   'Model and postprocessor value for `a` do '
                                   'not match. 1 != 2'):
            data_labeler.check_pipeline()
        ...
        with self.assertWarnsRegex(RuntimeWarning,
                                   'Preprocessor and postprocessor value for '
                                   '`b` do not match. 1 != 2'):
            data_labeler.check_pipeline()
    ...
    def test_fit(self, mock_open, mock_load_model, mock_base_processor):
        ...
        with self.assertRaisesRegex(ValueError,
                                    "`validation_split` must be >= 0 and less "
                                    "than 1.0"):
        ...
        output = data_labeler.fit(
            x=samples, y=labels, reset_weights=True)
        self.assertEqual(2, data_labeler.preprocessor.process.call_count)
        data_labeler._model.set_label_mapping.assert_not_called()
