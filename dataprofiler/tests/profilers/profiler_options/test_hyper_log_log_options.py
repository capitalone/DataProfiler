...
class TestHyperLogLogOptions(TestBooleanOption):
    ...
    def test_init(self):
        ...
        self.assertDictEqual({"is_enabled": False, "seed": 0}, option.properties)
        option = self.get_options(is_enabled=True)
        self.assertDictEqual({"is_enabled": True, "seed": 0}, option.properties)
        option = self.get_options(seed=10)
        self.assertDictEqual({"is_enabled": False, "seed": 10}, option.properties)
    ...
    def test_set(self):
        super().test_set()
        option = self.get_options()
        option.set({"is_enabled": True, "seed": 5})
        self.assertDictEqual({"is_enabled": True, "seed": 5}, option.properties)
    ...
    def test_validate_helper(self):
        ...
        option = self.get_options(seed=0)
        ...
        expected_error = [f"{optpth}.seed must be an int."]
