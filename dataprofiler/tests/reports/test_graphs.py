...
class TestPlotHistograms(unittest.TestCase):
    ...
    def test_bad_inputs(self, *mocks):
        ...
        with self.assertRaisesRegex(
            ...
            graphs.plot_histograms(
                self.profiler, column_names=["test"], column_inds=[1]
            )
        ...
        with self.assertRaisesRegex(
            ValueError, 'Column "a" is not found as a profiler ' "column"
        ):
    ...
    def test_specify_column(self, plot_col_mock, plt_mock):
        graphsplot = graphs.plot_histograms(self.profiler, column_names=["float"])
...
class TestPlotColHistogram(unittest.TestCase):
    ...
    def test_normal(self, plt_mock):
        self.assertIsInstance(graphs.plot_col_histogram(self.profiler), plt.Axes)
...
class TestPlotMissingValuesMatrix(unittest.TestCase):
    ...
    def test_bad_input(self, *mocks):
        ...
        with self.assertRaisesRegex(
            ValueError, "`profiler` must of type " "StructuredProfiler."
        ):
